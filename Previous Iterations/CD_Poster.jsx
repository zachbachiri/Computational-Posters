...
function draw() {
	var cities = [
	{
		...
		"density" : 963,
		"responses" : cbData.split(" zachbachiri ")
	...
	b.textFont('Univers LT Std\t45 Light');
	b.noStroke();

	for (var i = 0; i < cities.length; i++) {
		var x = b.round(b.map( cities[i].density, 0, 11000, 0, 3));
		if (x == 0) {
			var grid = b.width/4;
			var textSize = 38;
		...
		b.image("cb.jpg", 0, 0, b.width, b.height);

  		//b.text(grid, 100, 100*i, grid,grid);
  		...
  		for (var j = 0; j<rows; j++) {
  			for (var k = 0; k<columns; k++) {
  				var tbSize = grid-(grid*.1);
  				var tbX = (grid*k)+(grid*.05);
  				var tbY = (grid*j)+(grid*.05);
  				if(j+k == 0) {
  					b.textFont('Univers LT Std\t67 Bold Condensed');
  					b.textSize(textSize*1.7);
  					b.fill(255,255,255);
  					b.rect(grid*k, grid*j, grid, grid);
  					b.fill(255,50,50);
  					b.text("Tell Me How You Really Feel", tbX, tbY, tbSize, tbSize);
  					...
  					b.rect(grid*k, grid*j, grid, grid);
  					b.fill(255,50,50);
  					b.text(cities[i].name + "\n" + cities[i].date + "\n" + cities[i].location, tbX, tbY, tbSize, tbSize);

  				} else if (b.random(0,10)>5) {
  					b.textFont('Univers LT Std\t45 Light');
  					b.textSize(textSize);
  					b.fill(255,255,255);
  					b.rect(grid*k, grid*j, grid, grid);
  					b.fill(0,0,0);
  					textBoxes.push(b.text(cities[i].responses[b.round(b.random(cities[i].responses.length-1))], tbX, tbY, tbSize, tbSize));
  				}
  			}
  		...
  		for (var l = 0; l < textBoxes.length; l++) {
  			var txt = textBoxes[l].contents;
  			var txtArray = txt.split(/\W/);
  			var sentiment = getSentiment(txtArray);
  			if (sentiment > 0) {
  				b.typo(textBoxes[l], "appliedFont", "Univers LT Std\t45 Light Oblique");
  			}
...
function getSentiment(x) {
	...
	for (var i = 0; i < x.length; i++) {
		if (afinn.hasOwnProperty(x[i])) {
			score += Number(afinn[x[i]]);
			words = words+1.0;
		}
	}
	score = score/words;
