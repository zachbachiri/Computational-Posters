...
function draw() {
  ...
  //Loop through cities
    ...
    b.strokeWeight(0);
    b.rectMode(b.CORNER);
		var x = b.round(b.map( cities[i].density, 1000, 10000, 0, 3));
		if (x == 0) {
      var grid = b.width/2;
      var textSize = 45;
    ...
    if (x == 1) {
      ...
      //Draw rects and text boxes
  		...
  		for (var j = 0; j<rows; j++) {
  			for (var k = 0; k<columns; k++) {
  				var tbSize = grid-(grid*.15);
  				var tbX = (grid*k)+(grid*.075);
  				var tbY = (grid*j)+(grid*.075);
  				if(j+k == 0) {
      ...
      for (var c = 0; c<=columns; c++) {
        if (c == 0) {
          b.rect(grid*.025, b.height/2, grid*.05, b.height);
        } else if (c == columns) {
          b.rect(c*grid-grid*.025, b.height/2, grid*.05, b.height);
        } else {
          b.rect(c*grid, b.height/2, grid*.05, b.height);
        }
      }
      for (var  r= 0; r<=rows; r++) {
        if (r == 0) {
          b.rect(b.width/2, grid*.025, b.width, grid*.05);
        } else if (r == rows) {
          b.rect(b.width/2, r*grid-grid*.025, b.width, grid*.05);
        } else {
          b.rect(b.width/2, r*grid, b.width, grid*.05);
        }
      ...
      //Add sentiment skews
        ...
        /*
  			if (sentiment < -1) {
  				b.typo(textBoxes[l], "appliedFont", "Univers LT Std\t57 Condensed");
  			} else if (sentiment < 2) {

        } else if (sentiment < 5) {
          b.typo(textBoxes[l], "appliedFont", "Univers LT Std\t53 Extended");
        }*/

        //b.typo(textBoxes[l], "skew", sentiment*10);
        b.stroke(0,0,0);
        b.typo(textBoxes[l], "strokeWeight", b.abs(sentiment));
