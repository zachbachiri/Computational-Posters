...
function draw() {
  ...
  //Loop through cities
	for (var i = 0; i < cities.length; i++) {
		...
		var columns = b.round(b.map( cities[i].density, 500, 20000, 10, 40));
		var gridW = b.width / columns;
		var textSize = b.map(columns, 10, 40, 28, 12);
		b.textSize(textSize);
		...
		for (var j = 0; j<columns; j++) {
			var txt = cities[i].responses[b.round(b.random(cities[i].responses.length-1))];
	  		...
	  		var offSet = b.map(sentiment,-5,5,-75,75);

			var img = b.image("cb.jpg", -100, -150+offSet, b.width+200, b.height+300);
			img.geometricBounds = [0,j*gridW, b.height, (j+1)*gridW];

			b.rotate((b.PI/2));
			var txtBox = b.text(txt, (j+1)*gridW, 50, b.height-100, gridW);
			b.typo(txtBox, "justification", VerticalJustification.CENTER_ALIGN);
	  		b.typo(txtBox, "skew", sentiment*10);
	  		
	  		b.rotate(-(b.PI/2));
		}
		b.imageMode(b.CORNER);
		b.image("tellme.png", 50, 0, 300, 300);
		b.image("logo2.png", 50, b.height-300, 300, 300);
