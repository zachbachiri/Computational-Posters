  		...
  		for (var j = 0; j<rows; j++) {
  			for (var k = 0; k<columns; k++) {
  				...
  				if(j+k == 0) {
  					...
  					//var offsetX = b.random(-(grid*.25),(grid*.25));
  					//var offsetY = b.random(-(grid*.25),(grid*.25));
  					img = b.image("cb.jpg", 0+offsetX, 0+offsetY, b.width, b.height);
					img.geometricBounds = [gY, gX, gY+gSize, gX+gSize];
					//img.rotationAngle = b.random(-45,45);
  					b.textFont('Univers LT Std\t45 Light');
  					b.textSize(textSize);
  					b.fill(255,255,255);
  					b.opacity(b.rect(gX, gY, gSize, gSize), opc);
